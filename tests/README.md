# Welcome to pytest_ordered's test suite!
...
pytest_ordered aims to be out-the-box compatible with
...
that PyInstaller does need special help when compiling pytest_ordered
then that *help* goes in a *hook* file (see
`pytest_ordered/__pyinstaller/hook-pytest_ordered.py`).
...
changed the pytest_ordered source code.
...
This executable is just `pytest.main()` with pytest_ordered
...
Otherwise you may unintentionally use the original source pytest_ordered
...
If you've modified either the source code or the hooks for pytest_ordered
